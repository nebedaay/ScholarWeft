...
% scripts/sw-export.lua wraps the pre-rendered entry in this environment; it
% mirrors the "Bibliographic reference - body" paragraph style in the DOCX/ODT
% templates: a 1cm first line with a 0.75cm hanging indent (continuation lines
% at 1.75cm), no font change. Tune the indents here.
% A list environment (not \leftskip/\parindent) so the FIRST reference still
% hangs when the list directly follows a heading: LaTeX suppresses a
% paragraph's \parindent right after a heading, which silently flattened the
% first entry's hanging indent. leftmargin 1.75cm + itemindent -0.75cm gives a
% first line at 1cm and continuation lines at 1.75cm.
\newenvironment{swrefbody}{%
  \begin{list}{}{%
    \setlength{\leftmargin}{1.75cm}%
    \setlength{\labelwidth}{0pt}%
    \setlength{\labelsep}{0pt}%
    \setlength{\itemindent}{-0.75cm}%
    \setlength{\listparindent}{-0.75cm}%
...
\ifluatex
  \makeatletter
  % Load babel bare. Pandoc passes the document language as a \documentclass
  % option (from the note's `lang` property, or the export default the plugin
  % supplies), and LaTeX forwards unknown class options to babel — so the main
  % language (and its hyphenation) is set without naming it here. Loading babel
  % with NO language at all leaves \language at nohyphenation, which disables
  % hyphenation and stretches justified text, so the export always passes a
  % `lang` (see exportCompiler.ts / the "Default document language" setting).
...
% heading becomes \section{...} (or \chapter{...} for a book), while every
% unnumbered heading becomes the starred form \section*/\subsection*/...
...
% no heading can be double-numbered. There is therefore no titlesec/titletoc
% configuration here: LaTeX's own defaults style the headings.
SWTOKFIGURECOUNTER
