...
- **Notes** — three choices, set by the note's `endnotes` property. Both endnote choices compile the document exactly as the footnote path does — author notes and in-text citations all become real notes — then route them:
  ...
  - **Endnotes (as body paragraphs in the Notes section)** — the notes appear as visible paragraphs after the Notes heading, with superscript note numbers in the body and no native note objects. With per-chapter numbering they are grouped under their chapters (`## <chapter>`); with continuous numbering they form one list. Each note's citations are rendered inline within it (no page-bottom footnotes remain). **Markdown and LaTeX always use this form** (a `# Notes` section) — they have no native endnote objects.
...
| `endnotes` | Note rendering: `none` (default, footnotes), `native` (real endnote objects — **DOCX/ODT only**), or `body` (a visible `# Notes` section — divided by chapter when numbering is per-chapter, otherwise one list). Markdown and LaTeX have no native endnote objects, so they always use the `body` form |
