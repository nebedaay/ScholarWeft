...
- **Notes** — three choices, set by the note's `endnotes` property. Both endnote choices compile the document exactly as the footnote path does — author notes and in-text citations all become real notes — then route them:
  ...
  - **Endnotes (native)** — real endnote objects in **DOCX/ODT**, preceded by a level-1 **Notes** heading (it appears in the TOC). The word processor owns that stream and it cannot be divided by chapter: in **DOCX** its numbering restarts per chapter (per-chapter section breaks), while **ODT/LibreOffice numbers it continuously** — that format has no per-chapter endnote numbering. **LaTeX** gets native LaTeX endnotes (the `enotez` package): a Notes section at the end, split by chapter with the numbering restarted in each, and each note's citations rendered inline.
  - **Endnotes (as body paragraphs in the Notes section)** — a visible `# Notes` section whose notes are ordinary paragraphs, with superscript note numbers in the body. With per-chapter numbering they are grouped under their chapters (`## <chapter>`); with continuous numbering they form one list. Each note's citations are rendered inline within it (no page-bottom footnotes remain). Used by **DOCX/ODT** and **Markdown**.
...
| `endnotes` | Note rendering: `none` (default, footnotes), `native` (real endnote objects in DOCX/ODT; native LaTeX endnotes via the `enotez` package, split by chapter, in LaTeX), or `body` (a visible `# Notes` section — divided by chapter when numbering is per-chapter, otherwise one list; DOCX/ODT and Markdown) |
