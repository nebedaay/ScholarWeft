...
| `endnotes` | Note rendering: `none` (default, footnotes), `native` (real endnote objects in DOCX/ODT; native LaTeX endnotes via the `endnotes` package in LaTeX), or `body` (a visible `# Notes` section — divided by chapter when numbering is per-chapter, otherwise one list; DOCX/ODT and Markdown) |
