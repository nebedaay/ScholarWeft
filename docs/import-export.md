...
- **Notes** — three choices, set by the note's `endnotes` property. Both endnote choices compile the document exactly as the footnote path does — author notes and in-text citations all become real notes — then route them:
  ...
  - **Endnotes (native)** — real endnote objects in **DOCX/ODT**, preceded by a level-1 **Notes** heading (it appears in the TOC). The word processor owns that stream and it cannot be divided by chapter: in **DOCX** its numbering restarts per chapter (per-chapter section breaks), while **ODT/LibreOffice numbers it continuously** — that format has no per-chapter endnote numbering. **LaTeX** gets native LaTeX endnotes (the `enotez` package): a **Notes** chapter placed **before the bibliography**, split by chapter with the numbering restarted in each and each group headed **Notes for Chapter N** (or **Notes for *Title*** for an unnumbered chapter). The group headings stay out of the TOC, and the single **Notes** TOC entry points to the **first** notes page. Each note's citations are rendered inline, and the notes are set at body size with a small gap between them.
...
| `endnotes` | Note rendering: `none` (default, footnotes), `native` (real endnote objects in DOCX/ODT; native LaTeX endnotes via the `enotez` package, split by chapter and placed before the bibliography, in LaTeX), or `body` (a visible `# Notes` section — divided by chapter when numbering is per-chapter, otherwise one list; DOCX/ODT and Markdown) |
