...
- **Upgrading from an older version?** Notes made before `sw-related` existed have Zotero's tags and related links inside `related:`. Updating such a note moves them: anything Zotero still supplies is removed from `related:` (it now lives in `sw-related:`), while your own links — and any entry Zotero no longer has — stay where they are.
